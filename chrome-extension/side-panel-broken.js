...
document.getElementById('loginButton').addEventListener('click', async () => {
  ...
  try {
    ...
    const data = await window.ApiConfig.API.auth.login(username, password);
    ...
    } else if (error.message.includes('帳號或密碼錯誤')) {
      alert('帳號或密碼錯誤，請重新輸入');
    } else {
      alert('登入時發生錯誤，請稍後再試');
    }
...
document.addEventListener('click', async (e) => {
  if (e.target && e.target.id === 'sendData') {
    ...
    try {
      // 使用新的 API 配置獲取項目列表
      const data = await window.ApiConfig.API.items.list();
